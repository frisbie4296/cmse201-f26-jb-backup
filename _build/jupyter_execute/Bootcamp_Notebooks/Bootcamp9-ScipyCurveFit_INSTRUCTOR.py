...
popt, pcov = curve_fit(linear, my_x_data, my_y_data)
best_m = popt[0]
best_b = popt[1]

print('The best m is:',best_m,'The best b is:',best_b)

plt.scatter(my_x_data,my_y_data,label='Data')
plt.plot(my_x_data,linear(my_x_data,best_m,best_b),label = 'Fit')
plt.legend (loc='best')
plt.xlabel('x')
plt.ylabel('y')
...
popt,pcov = curve_fit(my_approximating_polynomial,x_array,my_y_data)
popt_sin,pcov_sin = curve_fit(my_sine_function,x_array,my_y_data)
y_vals_poly = my_approximating_polynomial(x_array,popt[0],popt[1],popt[2])
y_vals_sine = my_sine_function(x_array,popt_sin[0],popt_sin[1],popt_sin[2])

plt.scatter(x_array,my_y_data)
plt.plot(x_array,y_vals_poly,label='poly')
plt.plot(x_array,y_vals_sine,label='sine')
plt.legend(loc='best')
print(popt_sin)
print(popt)
print('poly:',rmse(x_array,my_y_data,y_vals_poly))
print('sine:',rmse(x_array,my_y_data,y_vals_sine))
...
popt_exp,pcov_exp = curve_fit(gen_exp,x_data_exp,y_data_exp)
print(popt_exp[0],popt_exp[1])

print('rmse=',rmse(x_data_exp,y_data_exp,gen_exp(x_data_exp,popt_exp[0],popt_exp[1])))

plt.plot(x_data_exp,gen_exp(x_data_exp,popt_exp[0],popt_exp[1]),label='fit')
plt.scatter(x_data_exp,y_data_exp,label='data')
plt.legend(loc='best')
