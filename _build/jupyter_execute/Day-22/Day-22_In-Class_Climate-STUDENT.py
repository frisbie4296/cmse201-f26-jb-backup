...
# # Day 22 In-class Assignment: Analyzing climate data
...
global_temp_recent = np.loadtxt('datasets/GLB.Ts.csv',skiprows=1,unpack=True,delimiter=',')
...
global_temp_bintanjan = np.loadtxt('datasets/bintanja2008.txt',skiprows=109,unpack=True)
...
vostok_temp = np.loadtxt("datasets/vostok-deutnat.txt",skiprows=111,unpack=True)
vostok_temp[1] *= -1.0
...
# You should mutiply the time information by -1 and add 2001 to get them in the right units
vostok_co2 = np.loadtxt("datasets/vostok-co2nat.txt",skiprows=155,unpack=True)
vostok_co2[0] *= -1.0
vostok_co2[0] += 2001.0
...
# 
# &#9989;&nbsp; To put the ice core measurements in context, adjust the subplot above so that it shows the most recent temperature, CO2 and CH4 measurements.  This can be done using either a red circle or a horizontal line. 
...
# 
# **Show this with a plot, and describe your outcomes below**.  (Hint: try using NumPy's [polyfit](https://docs.scipy.org/doc/numpy/reference/generated/numpy.polyfit.html) and/or SciPy's [curve_fit](https://docs.scipy.org/doc/scipy/reference/generated/scipy.optimize.curve_fit.html) methods!)
# 
