...
# 
# **NOTE** Remember from our work on in class that this isn’t the only ODE we need to include in our model. We also need to include the equation for the change in *length*, that is:
# 
# $$\frac{dl}{dt} = v$$
# 
...
# 
# For values to plug into your equations (I.e., $k$, $m$, $l_{\mathrm{unst}}$, $v_0$ (initial velocity), $l_0$ (initial length)), pick your own! Anything between 0.5-2.0 should work just fine for any of the variables. You can also try running the model for 30 seconds with timesteps of 0.1 seconds.
